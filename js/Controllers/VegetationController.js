angular.module('BAMApp.controllers').controller('VegetationController', ["$scope", "$rootScope", "dataService", "localStorageService", function ($scope, $rootScope, dataService, localStorageService) {
    ...
    this.function = {
        ...
        calculateFunctionSubtotal: function () {
            ...
            for (var property in this.model.currentFunction) {
                if (this.model.currentFunction.hasOwnProperty(property) && (property == 'weightedTreeScore' || property == 'weightedShrubScore' || property == 'weightedGrassAndGrassLikeScore' || property == 'weightedForbScore' || property == 'weightedFernScore' || property == 'weightedOtherScore')) {
    ...
    this.structure = {
        ...
        calculateDynamicWeightingScore: function (theObject, theObjectLower) {
            ...
            eval(`this.model.currentStructure.dynamicWeighting${theObject}Score = (benchmarks.${theObjectLower}Structure / sumOfBenchmarkScores).toFixed(2)`)
        },
        ...
        calculateUnweightedStructureScore: function (theObject, theObjectLower, observedValue) {
            ...
            if (observedValue == 0) {
                ...
                if (observedValue > eval(`benchmarks.${theObjectLower}Structure`)) {
                    returnValue = (100);
                } else {
                    returnValue = (
                        1.01 * (1 - Math.exp(-4.4 * Math.pow(observedValue / eval(`benchmarks.${theObjectLower}Structure`), 1.85))) * 100
                    );
                }
            }
